...
function ForceConverter({}) {
    const conversionUnits = {
        ...
        kp: 9.80665,
          ...
          const [convertFrom, setConvertFrom] = useState('N');
          const [convertTo, setConvertTo] = useState('N');
        ...
        const convertForceFunc = (value, from, to) =>{
          let result;
          // Convert input to Newtons (N)
          const valueInNewtons = value * (conversionUnits[from] || 0);
          // Convert Newtons (N) to the output unit
          result = valueInNewtons / (conversionUnits[to] || 1);
          setResultValue(result.toFixed(6)); // Set output value with precision
        }

        const shareFunc = () =>{
        let newLink =`${shareLinkBase}/physics-unit-converter?share=${true}&type=force-converter&val=${enteredValue}&from=${convertFrom}&to=${convertTo}&calc=0`
        ...
        const handleCalculateBtn = () => {
            ...
            if (isNaN(value)) {
              ...
              convertForceFunc(value, convertFrom, convertTo)
              shareFunc()
            }
        ...
        const modalDisplayFunc = () =>{
          ...
          useEffect(() => {
            ...
            if(share && type === 'force-converter'){
              setEnteredValue(Number(val));
              setConvertFrom(from);
              setConvertTo(to);
              setLoadSharedInputs(true)
              const value = parseFloat(val);   
              convertForceFunc(value, from, to)
            }else{
              setLoadSharedInputs(true)
            }
          }, [])
        ...
        <div className="relative bg-mattBlack text-leadGrey rounded-lg shadow dark:bg-gray-700">
          ...
          <form className="p-4 md:p-5 block font-open-sans">
            <div className="grid gap-4 mb-4 grid-cols-2">
              ...
              additionalClasses={''} 
              label={'Enter Conversion Unit'} 
              inputType={'number'}
              inputName={'conversionUnit'}
              inputId={''}
              additionInputClassName={''}
              inputPlaceholder={'Enter the Unit for conversion'}
              inputRequired={true}
